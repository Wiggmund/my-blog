...
export const PipeStringErrorMessages = {
	mustBeString: (data: ValidationArguments) => {
		return `Must be a string type, you provide [ ${checkForType(data.value)} ] type`;
	},
	shortString: (data: ValidationArguments) => {
		if (data.value === undefined) {
			return `You didn't provide value for $property`;
		}
		return `Is too short, minimum length is ${data.constraints[0]} characters you provide ${data.value.length || 0}`;
	},
...
export const PipeNumberErrorMessages = {
	mustBeNumber: (data: ValidationArguments) => {
		return `Must be a number type, you provide [ ${checkForType(data.value)} ] type`;
	},
